...
const executeFixtures = async () => {
    ...
    await csvdata.load(path.join(__dirname, "./usda/FD_GROUP.txt"), { parse: false, delimiter: "^", quote: "~" })
        .then(async data => {
            ...
            await csvdata.write(path.join(__dirname, 'out/food_group.csv'), docs, { header: 'id^name', delimiter: '^' })
        })
    ...
    await csvdata.load(path.join(__dirname, "./usda/FOOD_DES.txt"), { parse: false, delimiter: "^", quote: "~" })
        .then(async data => {
            ...
            await csvdata.write(path.join(__dirname, 'out/food.csv'), docs, { header: 'id^food_group_id^long_description^short_description^common_name', delimiter: '^' })
    ...
    await csvdata.load(path.join(__dirname, "./usda/NUTR_DEF.txt"), { parse: false, delimiter: "^", quote: "~" })
        .then(async data => {
            ...
            await csvdata.write(path.join(__dirname, 'out/nutrient.csv'), docs, { header: 'id^unit^name', delimiter: '^' })
        })
    ...
    await csvdata.load(path.join(__dirname, "./usda/NUT_DATA.txt"), { parse: false, delimiter: "^", quote: "~" })
        .then(async data => {
            ...
            nutrients = nutrients.map(nutrient => {
                ...
                return {
                    ...nutrient,
                    valueKcal: (100.0 / kCalNutrient) * nutrient.value
                }
            }).filter(u => u !== null)
            ...
            await csvdata.write(path.join(__dirname, 'out/food_nutrient.csv'), nutrients, { header: 'id^nutrient_id^food_id^value_per_g^value_per_kcal', delimiter: '^' })
    ...
    await csvdata.write(path.join(__dirname, 'out/food_measure_weight.csv'), foodMeasureWeights, { header: 'id^food_id^num_measures^measure^weight_in_g', delimiter: '^' })

    // Collections
    await csvdata.write(path.join(__dirname, 'out/collection.csv'), [{
        id: 1, name: 'Peas + Edamame', description: 'A collection of food I\'d eat on a busy day'
    }], { header: 'id^name^description', delimiter: '^' })

    // Collection Food Items
    await csvdata.write(path.join(__dirname, 'out/collection_food_item.csv'), [
        {
            id: 1,
    ...
    ], { header: 'id^food_id^collection_id', delimiter: '^' })
